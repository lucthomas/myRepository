...
public class Word
{
   ...
   public String scramble(String word)
   {
        ...
        for (int m = 0; m < word.length(); m++)
        {
            ...
            String fLetter = word.substring(i,i+1);
            int j = 1 + i + generator.nextInt(word.length()-i-1);
            
            String sLetter = word.substring(j,j+1);
            word = word.substring(0,i) + sLetter + word.substring(i+1,j) + fLetter + word.substring(j+1,word.length());
        }
